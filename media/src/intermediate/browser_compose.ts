...
export const main_interface: Example.Interface = {
  nodes: ["localRtcOutput", "master", "audio", "high", "medium", "low"],
  inputs: [
    Example.ffmpegCommand({
      // TODO: go back to AAC
      encode: Example.singleH264AacEncode({ ...Example.defaultEncode, audioCodec: "mp3" }),
      transport: Example.srtOutput({ port: 5001 })
    })],
  outputs: {
    audio: ["localRtcOutput"],
    video: ["localRtcOutput"],
    playlists: ["256-high", "256-low", "256-medium", "257-audio"]
  },
  description: "Add browser overlay to srt source"
};
